...
const YEAR = styled(Link)`
  ...
  font-weight: bold;
  &.decade {
    box-shadow: 0.25em 0.25em 0 #004988;
  }
  &:focus {
    background: red;
  }
...
function mostPopularWords(
  ...
  searchProp,
  minEntries = 10,
  exclude = ["the", "and", "a", "of", "be", "to", "for", "in", "on", "is", "as"]
) {
  let dictionary = {};
  list.forEach((entry) => {
    entry[searchProp]
...
function Clippings({
  ...
  let filterRE = filter
    ? new RegExp(filter ? filter.toLowerCase() : "*", "i")
    : { test: () => true };
  let yearsToShow = Object.entries(years).filter(
    (entry) =>
      !filter || entry[1].some((c) => filterRE.test(c.title + " " + c.desc))
  );
  ...
  if (ungrouped) {
    return (
      <CLIPPINGS
        className={isColumn && "isColumn" + isCaption && " isCaption"}
        {...props}
      >
        <Route
          path={`${url}/:year/:index`}
        ...
        {noTools ? null : (
          <div className="Tools">
            {periodSize !== 1 ? (
              <Link to={`${url}?s=${filter}`}>Years</Link>
            ) : (
              <strong>Years</strong>
            )}
            {periodSize === 10 ? (
              ...
              <Link to={`${url}?s=${filter}&group=decade`}>Decades</Link>
            )}
            {periodSize === 100 ? (
              <strong>Century</strong>
            ) : (
              <Link to={`${url}?s=${filter}&group=century`}>Century</Link>
            )}
            {year && (
              <Link
                to={`${url}/${
                  year.length === 4 ? `${year.slice(0, 3)}0s` : ""
                }?${queryString}`}
              >
                <strong>{year}</strong> x
              </Link>
            )}
            {searchOverride ? null : (
              <span>
                <input
                  className={`search ${filter && "notEmpty"}`}
                  placeholder="🔍"
                  value={filter}
                  onChange={(e) =>
                    history.push(
                      `${url}?s=${e.target.value}${
                        group ? `&group=${group}` : ""
                      }`
                    )
                  }
            ...
            <button onClick={(e) => setColumn(!isColumn)}>
              {isColumn ? "➡️" : "⬇️"}
            </button>
            <label style={{ display: "none" }}>
              ...
              <input
                type="checkbox"
                value={isCaption}
                onChange={(e) => setColumn(!isCaption)}
              />
            </label>
            <strong>{nRecords} entries</strong>
            <select
              onChange={(e) => history.push(`${url}?s=${e.target.value}`)}
            >
              <option></option>
              {Object.entries(
                mostPopularWords(Object.values(years).flat(), "title", 5)
              )
          ...
          {yearsToShow.length ? (
            yearsToShow
              .filter((e) =>
                year
                  ? year.length === 4
                    ? Number.parseInt(year) === Number.parseInt(e[0])
                    : e[0].includes(year.slice(0, 3))
                  : true
              )
              .map(([year, records]) => (
                <div className="groups">
                  <ol>
                    {records.map((record, index) =>
                      !filter ||
                      filterRE.test(record.title + " " + record.desc) ? (
                        <li key={record.localURL}>
                          <ClippingLink
                            to={record.localURL}
                            record={record}
                            size={size}
                          ...
                          >
                            {filter && filterRE.test(record.title) ? (
                              record.title.split(filterRE).map((t, i) => {
                                if (
                                  i !==
                                  record.title.split(filterRE).length - 1
                                )
                                  return (
                                    <>
                                      <span key={i}>{t}</span>
        ...
        {yearsToShow.length ? (
          yearsToShow
            ...
            .map(([year, records]) => (
              <div
                className="category"
                style={{ order: 2021 - Number.parseInt(year) }}
              >
                {showYear && (
                  <Link
                    className={`year ${year.length === 5 && "decade"}`}
                    to={`${url}/${year}?${queryString}`}
                    id={`y${year}`}
                  >
                    {year}
                  </Link>
                    ...
                    filterRE.test(record.title + " " + record.desc) ? (
                      <li key={record.localURL}>
                        ...
                        >
                          {filter && filterRE.test(record.title) ? (
                            record.title.split(filterRE).map((t, i) => {
                              if (i !== record.title.split(filterRE).length - 1)
                                return (
                                  <>
                                    <span key={i}>{t}</span>
                                    <mark>{filter}</mark>
